...
import { X, Calendar, GraduationCap, Plus, BookOpen, CheckCircle2, RefreshCw, EyeOff } from 'lucide-react';
...
export default function IntegrationsModal({ isOpen, onClose, primaryEmail }: IntegrationsModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const hiddenSources = JSON.parse(localStorage.getItem('daycraft_hidden_sources') || '[]');
      ...
      const updatedList = baseList.map(app => {
        // We check for tokens based on the integration ID
        ...
        const isHidden = hiddenSources.includes(app.id);

        if (hasToken || hasFakeToken) {
          return { 
            ...app, 
            isActive: !isHidden, 
            status: isHidden ? 'Hidden' : 'Connected', 
            lastSynced: 'Just now' 
          };
        }
  ...
  const handleAddNew = () => {
    ...
    // We put the provider right into the ID so the backend knows if it needs Classroom or Calendar keys!
    const uniqueId = `${newProvider}_${Date.now()}`; 
    ...
    setIntegrations(updatedList);
    localStorage.setItem('daycraft_integrations_list', JSON.stringify(updatedList.map(i => ({ id: i.id, provider: i.provider, name: i.name }))));
    setIsAdding(false);
    setNewName('');
  };
  ...
  const toggleIntegration = async (app: Integration) => {
    ...
    // FAKE CANVAS LOGIC (Only runs for Canvas)
    if (app.provider === 'canvas') {
    ...
    // REAL GOOGLE LOGIC (Handles both Calendar and Classroom)
    const hasToken = localStorage.getItem(`gcal_${app.id}_tokens`);

    if (!turningOn) {
      // HIDE IT: Add to hidden sources list
      const hidden = JSON.parse(localStorage.getItem('daycraft_hidden_sources') || '[]');
      if (!hidden.includes(app.id)) hidden.push(app.id);
      localStorage.setItem('daycraft_hidden_sources', JSON.stringify(hidden));

      setIntegrations(current => current.map(int => int.id === app.id ? { ...int, isActive: false, status: 'Hidden' } : int));
      window.location.reload(); 
      return;
    } else {
      // SHOW IT (If already connected and tokens exist)
      if (hasToken) {
        let hidden = JSON.parse(localStorage.getItem('daycraft_hidden_sources') || '[]');
        hidden = hidden.filter((id: string) => id !== app.id);
        localStorage.setItem('daycraft_hidden_sources', JSON.stringify(hidden));

        setIntegrations(current => current.map(int => int.id === app.id ? { ...int, isActive: true, status: 'Connected' } : int));
        window.location.reload(); 
        return;
      }

      // CONNECT IT (If brand new)
      setIntegrations(current => current.map(int => int.id === app.id ? { ...int, isSyncing: true, status: 'Contacting Google...' } : int));
      try {
        const fetchUrl = primaryEmail 
          ? `http://localhost:8080/api/auth/url?type=${app.id}&primaryEmail=${encodeURIComponent(primaryEmail)}`
          : `http://localhost:8080/api/auth/url?type=${app.id}`;
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4">
      <div className="bg-white w-full max-w-2xl rounded-[32px] p-8 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="space-y-4 overflow-y-auto pr-2 pb-4 flex-1 custom-scrollbar">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {app.isSyncing ? (
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">
          ...
          {!isAdding ? (
            <button onClick={() => setIsAdding(true)} className="w-full flex items-center justify-center gap-2 p-4 border-2 border-dashed border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50 text-slate-500 hover:text-indigo-600 font-bold rounded-2xl transition-all">
            ...
            <div className="p-5 border-2 border-indigo-100 bg-indigo-50/30 rounded-2xl space-y-4 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="flex gap-3">
                <select value={newProvider} onChange={(e) => setNewProvider(e.target.value as 'gcal' | 'canvas' | 'classroom')} className="bg-white border border-slate-200 text-slate-700 text-sm font-medium rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shrink-0">
                  <option value="gcal">Google Calendar</option>
                  <option value="canvas">Canvas LMS</option>
                  <option value="classroom">Google Classroom</option>
                </select>
                <input type="text" placeholder="e.g., West Valley Canvas" value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-700 text-sm font-medium rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" autoFocus />
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
                <button onClick={handleAddNew} disabled={!newName.trim()} className="px-4 py-2 text-sm font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Add Account</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="pt-6 border-t border-slate-100 flex justify-end shrink-0">
          <button onClick={onClose} className="px-8 py-3 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl shadow-lg transition-colors">Done</button>
        </div>
      </div>
    </div>
  );
